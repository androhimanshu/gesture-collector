...
public final class TouchRecorder {
    ...
    private TouchSample sample(MotionEvent e, int p, long time, int h) {
        ...
        // ── raw_x / raw_y fix ────────────────────────────────────────────────
        // API 29+ exposes per-pointer raw coords for both current and historical
        // samples. Below API 29, getRawX/Y only returns the primary pointer;
        // we approximate all other pointers via the view→screen offset.
        float rawX, rawY;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            rawX = cur ? e.getRawX(p) : e.getHistoricalRawX(p, h);
            rawY = cur ? e.getRawY(p) : e.getHistoricalRawY(p, h);
        } else {
            // Offset is constant for a non-moving view
